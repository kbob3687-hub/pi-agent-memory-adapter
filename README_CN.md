...
> 本仓库是 [TencentDB-Agent-Memory PR #1070](https://github.com/TencentCloud/TencentDB-Agent-Memory/pull/1070) 的独立 companion 实现。PR 当前仍是 Open 状态；这里提供最方便的独立体验入口。
...
L1–L3 由 MemoryCore 根据已采集的对话异步生成；适配器不伪造、不编辑它们。`/tdai-memory-forget` 可按需删除匹配的 L1 原子记忆与 L0 会话消息，但 L2/L3 画像仅能在服务端移除。除 setup 向导可选创建私有 Agent 外，不会自动创建 Team/Agent，也不迁移历史聊天。投递语义是 at-least-once：如果服务端已经接受请求但响应丢失，之后重试可能产生重复。
...
这个命令只补投当前配置身份的普通记忆队列，并显示已投递、仍 pending、dead 和 invalid 的数量。

Skills 的投递默认不会自动重试。因为网络超时或连接中断时，服务端可能已经收到了这批内容；再次发送可能导致 Skill 学习输入重复。确认服务端确实没有收到，或者你明确接受重复风险后，可以执行：

```text
/tdai-memory-retry-skills
```

如果 Skill 因确定性的 4xx 错误进入 `.json.dead`，确认不再需要这些记录后，可以执行：

```text
/tdai-memory-cleanup-skills
```

清理命令只删除当前 endpoint、service、team、agent、user 身份下的 `.dead` 文件，不会删除 `uncertain` 文件。`uncertain` 文件需要先人工判断是否应该重试。

## 遗忘记忆

要删除记住的事实以及背后的原始对话证据，可以执行：

```text
/tdai-memory-forget <关键词>
```

它会按关键词搜索 L1 原子记忆与 L0 会话消息，展示脱敏预览，等你确认后再删除。此命令不覆盖 L2 场景笔记和 L3 画像——这两层请在 Memory Hub 中移除。
