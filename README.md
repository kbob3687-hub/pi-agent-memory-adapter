...
> Standalone companion implementation for [TencentDB-Agent-Memory PR #1070](https://github.com/TencentCloud/TencentDB-Agent-Memory/pull/1070). The PR is currently open; this repository is the easiest way to inspect and run the Pi adapter independently.

[![Pi adapter CI](https://github.com/kbob3687-hub/pi-agent-memory-adapter/actions/workflows/ci.yml/badge.svg)](https://github.com/kbob3687-hub/pi-agent-memory-adapter/actions/workflows/ci.yml)
...
L1–L3 are generated by MemoryCore from the captured conversation stream; the adapter does not fabricate or edit them. `/tdai-memory-forget` can delete matching L1 memories and L0 conversation messages on request, but L2/L3 profiles are only removable server-side. Apart from the setup wizard optionally creating a private Agent, it does not auto-create teams/agents or migrate historical chats. Delivery is at-least-once: if the service accepts a record but its response is lost, a retry can create a duplicate.
...
To remove remembered facts and the raw conversation evidence behind them, run:

```text
/tdai-memory-forget <keyword>
```

It searches L1 atomic memories and L0 conversation messages for the keyword, shows a redacted preview, and deletes the matches after you confirm. L2 scenario notes and the L3 profile are not covered by this command — remove those from Memory Hub.
